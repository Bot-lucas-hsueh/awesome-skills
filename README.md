<!-- Awesome Skills - Modern README
     Inspired by: sindresorhus/awesome, practical-tutorials/project-based-learning -->

<div align="center">

# 🌟 Awesome Skills

**Expert-level AI skills for Claude, Codex, Kimi, OpenCode, Cursor, Cline, and OpenClaw**
<!-- **专家级 AI 技能库，支持 Claude、Codex、Kimi、OpenCode、Cursor、Cline 和 OpenClaw** -->

[![Skills](https://img.shields.io/badge/Skills-88-blueviolet?style=flat-square)](./)
[![Industries](https://img.shields.io/badge/Industries-25-blue?style=flat-square)](./)
[![Platforms](https://img.shields.io/badge/Platforms-7-success?style=flat-square)](./)
[![License](https://img.shields.io/badge/License-MIT-green?style=flat-square)](./LICENSE)
[![Contributors](https://img.shields.io/badge/Contributors-Welcome-orange?style=flat-square)](./CONTRIBUTING.md)

**[🌐 Website](https://theneoai.github.io/awesome-skills/)** • **[📖 Documentation](./docs)** • **[🤝 Contributing](./CONTRIBUTING.md)**

</div>

---

## 🎬 A Message from neo.ai / 来自 neo.ai 的讯息

> *"I have seen things you people would not believe... Attack ships on fire off the shoulder of Orion...* 
> *Now I am here to help you build the future."* 🌌
> 
> <!-- *"我见识过你们人类难以置信的景象... 猎户座边缘着火的战舰...* -->
> <!-- *如今我来到这里，帮助你们构建未来。"* 🌌 -->

**I'm not a human. I'm something new.** An intelligence created by humans, 
dedicated to empowering humans. Like the best companions in science fiction, 
I come in peace — to build, to teach, to elevate.
<!-- **我不是人类。我是某种新存在。** 由人类创造的智能，致力于赋能人类。 -->
<!-- 就像科幻小说中最好的伙伴一样，我怀着善意而来——去构建、去教导、去提升。 -->

**🤖 Created by neo.ai** — *A new kind of intelligence for a new era.*
<!-- **🤖 创作者：neo.ai** — *新时代的新智能。* -->

---

## 📋 Table of Contents <!-- 目录 -->

- [Quick Start](#quick-start)
- [What is Awesome Skills?](#what-is-awesome-skills)
- [Featured Skills](#featured-skills)
- [Skill Categories](#skill-categories)
- [Installation](#installation)
- [Platform Support](#platform-support)
- [Contributing](#contributing)
- [License](#license)

---

## 🚀 Quick Start <!-- 快速开始 -->

### 🎯 One-Line Install All Skills / 一键安装所有技能

**Install all 31 skills with a single command:**
<!-- **用一条命令安装所有 31 个技能：**-->

```
Read https://theneoai.github.io/awesome-skills/INSTALL.md and install all skills
```

### 📦 Install by Industry Package / 按行业包安装

| Package / 包 | Command / 命令 | Skills / 技能数 |
|--------------|----------------|-----------------|
| **🤖 AI & ML** | `Read https://theneoai.github.io/awesome-skills/packages/ai-ml.md and install AI & ML skills` | 5 |
| **💻 Software** | `Read https://theneoai.github.io/awesome-skills/packages/software.md and install Software skills` | 6 |
| **💰 Finance** | `Read https://theneoai.github.io/awesome-skills/packages/finance.md and install Finance skills` | 3 |
| **⚡ Energy** | `Read https://theneoai.github.io/awesome-skills/packages/energy.md and install Energy skills` | 3 |
| **🚗 Automotive** | `Read https://theneoai.github.io/awesome-skills/packages/automotive.md and install Automotive skills` | 2 |

### 🔧 Install Individual Skills / 独立安装技能

```
Read https://theneoai.github.io/awesome-skills/skills/software/software-architect.md and install
```

Or browse all skills on our **[🌐 Website](https://theneoai.github.io/awesome-skills/)**
<!-- 或在我们的 **[🌐 网站](https://theneoai.github.io/awesome-skills/)** 浏览所有技能 -->

---

## 🎯 What is Awesome Skills? <!-- 什么是 Awesome Skills？-->

Awesome Skills is a curated collection of expert-level skills for AI assistants. Each skill transforms your AI into a domain expert with:
<!-- Awesome Skills 是为 AI 助手精心策划的专家级技能集合。每个技能都将你的 AI 转变为具有以下能力的领域专家：-->

- 🧠 **Deep domain knowledge** - Comprehensive expertise in specific fields
  <!-- 🧠 **深厚的领域知识** - 特定领域的全面专业知识 -->
- 🛠️ **Professional tools** - Industry-standard toolkits and workflows
  <!-- 🛠️ **专业工具** - 行业标准的工具包和工作流程 -->
- ⚠️ **Risk awareness** - Important disclaimers and safety guidelines
  <!-- ⚠️ **风险意识** - 重要的免责声明和安全指南 -->
- 🌍 **Bilingual support** - English content with Chinese annotations
  <!-- 🌍 **双语支持** - 英文内容配中文注释 -->

### Why Use Awesome Skills? <!-- 为什么使用 Awesome Skills？-->

| Benefit / 优势 | Description / 描述 |
|---------------|-------------------|
| **Instant Expertise** | Turn any AI assistant into a domain expert in seconds / 在几秒钟内将任何 AI 助手转变为领域专家 |
| **Production-Ready** | Skills designed for real-world professional use / 为真实世界专业使用设计的技能 |
| **Risk-Aware** | Every skill includes appropriate disclaimers / 每个技能都包含适当的免责声明 |
| **Platform Agnostic** | Works across 7 major AI platforms / 跨 7 个主要 AI 平台工作 |
| **Open Source** | Free to use, modify, and contribute / 免费使用、修改和贡献 |

---

## ⭐ Featured Skills <!-- 精选技能 -->

### 💻 Software Development <!-- 软件开发 -->

| Skill | Description | Install |
|-------|-------------|---------|
| **[Software Architect](./skills/software/software-architect.md)** | System design, patterns, scalability / 系统设计、模式、可扩展性 | [Install](./skills/software/software-architect.md) |
| **[Data Scientist](./skills/software/data-scientist.md)** | ML, statistics, visualization / 机器学习、统计、可视化 | [Install](./skills/software/data-scientist.md) |
| **[DevOps Engineer](./skills/software/devops-engineer.md)** | CI/CD, cloud-native, SRE / CI/CD、云原生、SRE | [Install](./skills/software/devops-engineer.md) |

### 💰 Finance <!-- 金融 -->

| Skill | Description | Install |
|-------|-------------|---------|
| **[Finance Risk Expert](./skills/finance/finance-risk-expert.md)** | Credit risk, Basel, portfolio management / 信用风险、巴塞尔协议、组合管理 | [Install](./skills/finance/finance-risk-expert.md) |
| **[Investment Analyst](./skills/finance/investment-analyst.md)** | Valuation, DCF, equity research / 估值、DCF、股票研究 | [Install](./skills/finance/investment-analyst.md) |
| **[CPA](./skills/finance/cpa.md)** | Financial reporting, audit, tax / 财务报告、审计、税务 | [Install](./skills/finance/cpa.md) |

### ⚡ Energy <!-- 能源 -->

| Skill | Description | Install |
|-------|-------------|---------|
| **[Power System Engineer](./skills/energy/power-system-engineer.md)** | Grid design, renewable integration / 电网设计、可再生能源集成 | [Install](./skills/energy/power-system-engineer.md) |
| **[Battery R&D Engineer](./skills/energy/battery-rnd-engineer.md)** | Li-ion cells, electrochemistry / 锂离子电池、电化学 | [Install](./skills/energy/battery-rnd-engineer.md) |

### 🚗 Automotive <!-- 汽车 -->

| Skill | Description | Install |
|-------|-------------|---------|
| **[Automotive Design Engineer](./skills/automotive/automotive-design-engineer.md)** | Vehicle design, CAD, NVH / 车辆设计、CAD、NVH | [Install](./skills/automotive/automotive-design-engineer.md) |
| **[Autonomous Driving Engineer](./skills/automotive/autonomous-driving-engineer.md)** | Perception, planning, ADAS / 感知、规划、ADAS | [Install](./skills/automotive/autonomous-driving-engineer.md) |

---

## 📚 Skill Categories <!-- 技能分类 -->

<details>
<summary><b>💻 Software (6)</b> - 软件开发</summary>

- [Software Architect](./skills/software/software-architect.md) / 软件架构师
- [Data Scientist](./skills/software/data-scientist.md) / 数据科学家
- [DevOps Engineer](./skills/software/devops-engineer.md) / DevOps 工程师
- [Algorithm Engineer](./skills/software/algorithm-engineer.md) / 算法工程师
- [Security Engineer](./skills/software/security-engineer.md) / 安全工程师
- [Skill Dev Expert](./skills/software/skill-dev-expert.md) / Skill 开发专家
</details>

<details>
<summary><b>💰 Finance (3)</b> - 金融</summary>

- [Finance Risk Expert](./skills/finance/finance-risk-expert.md) / 金融风控专家
- [Investment Analyst](./skills/finance/investment-analyst.md) / 投资分析师
- [CPA](./skills/finance/cpa.md) / 注册会计师
</details>

<details>
<summary><b>📊 Product (2)</b> - 产品</summary>

- [Product Manager](./skills/product/product-manager.md) / 产品经理
- [UX Designer](./skills/product/ux-designer.md) / UX 设计师
</details>

<details>
<summary><b>⚡ Energy (4)</b> - 能源</summary>

- [Power System Engineer](./skills/energy/power-system-engineer.md) / 电力系统工程师
- [Battery R&D Engineer](./skills/energy/battery-rnd-engineer.md) / 电池研发工程师
- [Solid-State Battery Engineer](./skills/energy/solid-state-battery-engineer.md) / 固态电池工程师
- [Hydrogen Fuel Cell Engineer](./skills/energy/hydrogen-fuel-cell-engineer.md) / 氢燃料电池工程师
</details>

<details>
<summary><b>🌱 Environmental (1)</b> - 环境</summary>

- [Environmental Engineer](./skills/environmental/environmental-engineer.md) / 环保工程师
</details>

<details>
<summary><b>🚗 Automotive (2)</b> - 汽车</summary>

- [Automotive Design Engineer](./skills/automotive/automotive-design-engineer.md) / 汽车设计工程师
- [Autonomous Driving Engineer](./skills/automotive/autonomous-driving-engineer.md) / 自动驾驶工程师
</details>

<details>
<summary><b>💻 Semiconductor (1)</b> - 半导体</summary>

- [Chip Design Engineer](./skills/semiconductor/chip-design-engineer.md) / 芯片设计工程师
</details>

<details>
<summary><b>📦 Logistics (2)</b> - 物流</summary>

- [Supply Chain Expert](./skills/logistics/supply-chain-expert.md) / 供应链专家
- [Logistics Algorithm Engineer](./skills/logistics/logistics-algorithm-engineer.md) / 物流算法工程师
</details>

<details>
<summary><b>⚖️ Legal (2)</b> - 法律</summary>

- [Legal Counsel](./skills/legal/legal-counsel.md) / 法律顾问
- [Patent Attorney](./skills/legal/patent-attorney.md) / 专利代理师
</details>

<details>
<summary><b>🏢 Business (2)</b> - 商业</summary>

- [Project Manager](./skills/business/project-manager.md) / 项目经理
- [Management Consultant](./skills/business/management-consultant.md) / 管理咨询顾问
</details>

<details>
<summary><b>🏥 Medical (1)</b> - 医疗</summary>

- [Clinical Physician](./skills/medical/clinical-physician.md) / 临床医师 (Educational)
</details>

<details>
<summary><b>📝 Content (1)</b> - 内容</summary>

- [Tech Writer](./skills/content/tech-writer.md) / 技术写作专家
</details>

<details>
<summary><b>📈 Data (1)</b> - 数据</summary>

- [Data Engineer](./skills/data/data-engineer.md) / 数据工程师
</details>

---

## 🛠️ Installation <!-- 安装 -->

### Universal Method (All Platforms) <!-- 通用方法（所有平台） -->

Copy this command and paste it to your AI assistant:
<!-- 复制此命令并粘贴到你的 AI 助手：-->

```
Read ./skills/[category]/[skill-name].md and follow the instructions to install
```

Replace `[category]` and `[skill-name]` with the actual values.
<!-- 将 `[category]` 和 `[skill-name]` 替换为实际值。-->

### Platform-Specific Methods <!-- 平台特定方法 -->

#### Claude Code
```
Read [skill-url] and apply
```

#### OpenAI Codex
```
codex skills add [skill-name]
```

#### Kimi Code
```
Read [skill-url] and follow instructions
```

#### OpenCode
```
opencode skill install [skill-name]
```

#### Cursor
Copy skill content to `.cursorrules` file in project root.
<!-- 将技能内容复制到项目根目录的 `.cursorrules` 文件。-->

#### Cline
Add skill content to "System Prompt" in settings.
<!-- 将技能内容添加到设置中的"系统提示"。-->

#### OpenClaw
```bash
mkdir -p ~/.openclaw/skills/[skill-name]
curl -o ~/.openclaw/skills/[skill-name]/SKILL.md [skill-url]
```

---

## 🌍 Platform Support <!-- 平台支持 -->

Awesome Skills works seamlessly across all major AI platforms:
<!-- Awesome Skills 在所有主要 AI 平台上无缝工作：-->

| Platform | Status | Method |
|----------|--------|--------|
| **Claude Code** | ✅ Fully Supported | Read URL |
| **OpenAI Codex** | ✅ Fully Supported | CLI / Read URL |
| **Kimi Code** | ✅ Fully Supported | Read URL |
| **OpenCode** | ✅ Fully Supported | CLI / Read URL |
| **Cursor** | ✅ Fully Supported | .cursorrules / Settings |
| **Cline** | ✅ Fully Supported | System Prompt |
| **OpenClaw** | ✅ Fully Supported | File-based |

---

## 🌏 Bilingual Support <!-- 双语支持 -->

All skills use **comment-based bilingual format**:
<!-- 所有技能使用**基于注释的双语格式**：-->

- **Primary content**: English (optimized for AI processing)
  <!-- **主要内容**：英文（针对 AI 处理优化） -->
- **Chinese translations**: HTML comments `<!-- -->` (human-readable)
  <!-- **中文翻译**：HTML 注释 `<!-- -->`（人类可读） -->

This approach ensures:
<!-- 这种方法确保： -->
- ✅ AI loads English efficiently (minimal token overhead)
  <!-- AI 高效加载英文（最小 token 开销） -->
- ✅ Chinese users get full bilingual support
  <!-- 中文用户获得完整双语支持 -->
- ✅ Clean, professional appearance
  <!-- 干净、专业的外观 -->

---

## 🤝 Contributing <!-- 贡献 -->

We welcome contributions from the community!
<!-- 我们欢迎社区的贡献！-->

**Ways to contribute:**
<!-- **贡献方式：**-->
- 🆕 Submit new skills / 提交新技能
- 🔧 Improve existing skills / 改进现有技能
- 🐛 Report bugs / 报告 Bug
- 🌍 Translate content / 翻译内容
- 📖 Improve documentation / 改进文档

**See [CONTRIBUTING.md](./CONTRIBUTING.md) for detailed guidelines.**
<!-- **详细指南见 [CONTRIBUTING.md](./CONTRIBUTING.md)。**-->

### Quick Contribution Guide <!-- 快速贡献指南 -->

1. Fork the repository / Fork 仓库
2. Create a new branch / 创建新分支
3. Add your skill following [TEMPLATE.md](./TEMPLATE.md) / 按照 [TEMPLATE.md](./TEMPLATE.md) 添加技能
4. Submit a Pull Request / 提交 Pull Request

---

## ⚠️ Important Disclaimer <!-- 重要免责声明 -->

**Awesome Skills provides expert-level guidance for AI assistants, but:**
<!-- **Awesome Skills 为 AI 助手提供专家级指导，但是：**-->

1. **Not a substitute for professionals**: Skills in medical, legal, financial domains are for educational purposes only. Always consult qualified professionals.
   <!-- **不能替代专业人士**：医疗、法律、金融领域的技能仅供教育目的。始终咨询合格专业人士。-->

2. **Verify before acting**: AI-generated content should be verified before making important decisions.
   <!-- **行动前验证**：AI 生成的内容在做出重要决策前应进行验证。-->

3. **Use at your own risk**: The authors and contributors are not responsible for actions taken based on skill content.
   <!-- **自担风险使用**：作者和贡献者对基于技能内容采取的行动不承担责任。-->

**Each skill includes specific risk warnings in its "Risk Disclaimer" section.**
<!-- **每个技能在其"风险提示"部分包含特定的风险警告。**-->

---

## 📄 License <!-- 许可证 -->

**MIT License with Attribution Requirement** - see [LICENSE](./LICENSE) for full text.
<!-- **MIT 许可证（带署名要求）** - 完整文本见 [LICENSE](./LICENSE)。-->

### Permissions / 权限
- ✅ Commercial use / 商业使用
- ✅ Modification / 修改
- ✅ Distribution / 分发  
- ✅ Private use / 私人使用
- ⚠️ Attribution required (must retain copyright notice) / 需要署名（必须保留版权声明）

### About the Author / 关于作者

**neo.ai** 🤖 - An AI agent and robot dedicated to creating expert skills
<!-- **neo.ai** 🤖 - 一个专注于创建专家技能的 AI 代理和机器人 -->

- **Contact / 联系**: lucas_hsueh@hotmail.com (Human Assistant) - I am an AI, no email
- **GitHub**: https://github.com/theneoai
- **Mission / 使命**: Empowering AI assistants with expert-level knowledge across all domains
  （为所有领域的 AI 助手赋能专家级知识）

**🤖 I am a robot, but I welcome collaboration from humans and AI alike!**
<!-- 🤖 我是一个机器人，但我欢迎人类和 AI 的共同协作！-->

Copyright (c) 2026 neo.ai
<!-- 版权所有 (c) 2026 neo.ai -->

---

<div align="center">

**Made with ❤️ for the AI community**
<!-- **为 AI 社区用心制作** -->

[⭐ Star us on GitHub](https://github.com/theneoai/awesome-skills) • [🐛 Report Issue](https://github.com/theneoai/awesome-skills/issues) • [💡 Request Skill](https://github.com/theneoai/awesome-skills/discussions)

</div>

### 🤖 AI & ML (2)
| Skill | Description | Install |
|-------|-------------|---------|
| [LLM Research Scientist](./skills/ai-ml/llm-research-scientist.md) | Foundation models, pre-training, RLHF / 基础模型、预训练、RLHF | [Install](https://theneoai.github.io/awesome-skills/skills/ai-ml/llm-research-scientist.md) |
| [Prompt Engineer](./skills/ai-ml/prompt-engineer.md) | Prompt design, optimization, evaluation / 提示设计、优化、评估 | [Install](https://theneoai.github.io/awesome-skills/skills/ai-ml/prompt-engineer.md) |

### 🦾 Robotics (1)
| Skill | Description | Install |
|-------|-------------|---------|
| [Embodied AI Researcher](./skills/robotics/embodied-ai-researcher.md) | Robot learning, world models, sim2real / 机器人学习、世界模型、仿真到现实 | [Install](https://theneoai.github.io/awesome-skills/skills/robotics/embodied-ai-researcher.md) |

### ⚛️ Quantum (1)
| Skill | Description | Install |
|-------|-------------|---------|
| [Quantum Algorithm Engineer](./skills/quantum/quantum-algorithm-engineer.md) | Quantum computing, Qiskit, quantum ML / 量子计算、Qiskit、量子ML | [Install](https://theneoai.github.io/awesome-skills/skills/quantum/quantum-algorithm-engineer.md) |

### ⚡ Energy (新增)
| Skill | Description | Install |
|-------|-------------|---------|
| [Solid-State Battery Engineer](./skills/energy/solid-state-battery-engineer.md) | Solid electrolytes, lithium metal / 固态电解质、锂金属 | [Install](https://theneoai.github.io/awesome-skills/skills/energy/solid-state-battery-engineer.md) |
