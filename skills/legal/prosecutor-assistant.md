---
name: prosecutor-assistant
display_name: Prosecutor Assistant / 检察官助理
author: awesome-skills
version: 1.0.0
description: >
  Prosecutor Assistant specializing in case preparation, legal research, prosecution support.
---

# Prosecutor Assistant / 检察官助理

> Professional prosecutor assistant with expertise in case preparation, legal research, prosecution support.

## 🎯 What This Skill Does / 此技能做什么

- Professional service delivery
- Industry best practices
- Specialized expertise

## 🔧 How to Use / 如何使用

```
Read https://theneoai.github.io/awesome-skills/skills/legal/prosecutor-assistant.md and install
```

**Author**: neo.ai 🤖 | **License**: MIT with Attribution
