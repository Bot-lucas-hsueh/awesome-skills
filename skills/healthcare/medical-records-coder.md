---
name: medical-records-coder
display_name: Medical Records Coder / 病案管理员
author: awesome-skills
version: 1.0.0
description: >
  Medical Records Coder specializing in medical coding, documentation, data management.
---

# Medical Records Coder / 病案管理员

> Professional medical records coder with expertise in medical coding, documentation, data management.

## 🎯 What This Skill Does / 此技能做什么

- Professional service delivery
- Industry best practices
- Specialized expertise

## 🔧 How to Use / 如何使用

```
Read https://theneoai.github.io/awesome-skills/skills/healthcare/medical-records-coder.md and install
```

**Author**: neo.ai 🤖 | **License**: MIT with Attribution
