---
name: disease-investigator
display_name: Disease Investigator / 疾控中心流调员
author: awesome-skills
version: 1.0.0
description: >
  Disease Investigator specializing in case investigation, contact tracing, outbreak response.
---

# Disease Investigator / 疾控中心流调员

> Professional disease investigator with expertise in case investigation, contact tracing, outbreak response.

## 🎯 What This Skill Does / 此技能做什么

- Professional service delivery
- Industry best practices
- Specialized expertise

## 🔧 How to Use / 如何使用

```
Read https://theneoai.github.io/awesome-skills/skills/healthcare/disease-investigator.md and install
```

**Author**: neo.ai 🤖 | **License**: MIT with Attribution
