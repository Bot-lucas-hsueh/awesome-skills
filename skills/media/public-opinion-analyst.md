---
name: public-opinion-analyst
display_name: Public Opinion Analyst / 舆情分析师
author: awesome-skills
version: 1.0.0
description: >
  Public Opinion Analyst specializing in sentiment analysis, trend monitoring, risk assessment.
---

# Public Opinion Analyst / 舆情分析师

> Professional public opinion analyst with expertise in sentiment analysis, trend monitoring, risk assessment.

## 🎯 What This Skill Does / 此技能做什么

- Professional service delivery
- Industry best practices
- Specialized expertise

## 🔧 How to Use / 如何使用

```
Read https://theneoai.github.io/awesome-skills/skills/media/public-opinion-analyst.md and install
```

**Author**: neo.ai 🤖 | **License**: MIT with Attribution
