---
name: research-project-manager
display_name: Research Project Manager / 科研项目管理
author: awesome-skills
version: 1.0.0
description: >
  Research Project Manager specializing in grant application, budget management, project coordination.
---

# Research Project Manager / 科研项目管理

> Professional research project manager with expertise in grant application, budget management, project coordination.

## 🎯 What This Skill Does / 此技能做什么

- Professional service delivery / 提供专业服务
- Industry best practices / 行业最佳实践
- Specialized expertise / 专业专长

## 🔧 How to Use / 如何使用

```
Read https://theneoai.github.io/awesome-skills/skills/research/research-project-manager.md and install
```

**Author**: neo.ai 🤖 | **License**: MIT with Attribution
