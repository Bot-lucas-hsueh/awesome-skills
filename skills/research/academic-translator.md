---
name: academic-translator
display_name: Academic Translator / 学术翻译
author: awesome-skills
version: 1.0.0
description: >
  Academic Translator specializing in paper polishing, professional translation, language editing.
---

# Academic Translator / 学术翻译

> Professional academic translator with expertise in paper polishing, professional translation, language editing.

## 🎯 What This Skill Does / 此技能做什么

- Professional service delivery / 提供专业服务
- Industry best practices / 行业最佳实践
- Specialized expertise / 专业专长

## 🔧 How to Use / 如何使用

```
Read https://theneoai.github.io/awesome-skills/skills/research/academic-translator.md and install
```

**Author**: neo.ai 🤖 | **License**: MIT with Attribution
